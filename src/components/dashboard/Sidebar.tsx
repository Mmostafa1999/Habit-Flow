...
const Sidebar = ({ isMobile = false, onClose }: SidebarProps) => {
    ...
    const navItems = [
        ...
        { label: 'Statistics', icon: <BarChart2 className="w-5 h-5" />, path: '/dashboard/statistics' },
        { label: 'Profile', icon: <User className="w-5 h-5" />, path: '/dashboard/profile' },
        { label: 'AI Assistant', icon: <Bot className="w-5 h-5" />, path: '/dashboard/assistant' },
    ];
    ...
    return (
        <aside className={`bg-white shadow-md flex flex-col ${isMobile ? 'w-full' : 'w-60'} h-full`}>
